...
namespace EStoria.Unit.Tests
{
	public class DomainEventSaverSetup : WithSubject<DomainEventSaver>
	{
		...
		protected static Subject<DomainEvent> DomainEvents;
		protected static DomainEvent SomeDomainEvent;
		protected static Exception Exception;

		Establish context = () =>
		{
			Events = new List<CommittedEvent>();
			DomainEvents = new Subject<DomainEvent>();
			SomeDomainEvent = new DomainEvent { AggregateId = "aggregateId", Event = "some value" };

			Configure(registrar => registrar.For<IObservable<DomainEvent>>().Use(DomainEvents));
	...
	{			
		...
		It should_save_the_event_on_the_store = () => The<IEventStore>().WasToldTo(store => store.Append("aggregateId", SomeDomainEvent.Event)).OnlyOnce();
		It should_publish_the_committed_event_corresponding_to_the_event = () => Events.Select(evt => evt.Data).Should().ContainInOrder(SomeDomainEvent.Event);
	}
