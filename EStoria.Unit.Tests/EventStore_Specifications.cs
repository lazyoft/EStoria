...
namespace EStoria.Unit.Tests
{
	...
	public class When_saving_a_single_event : WithSubject<EventStore>
	{
		...
		It should_assign_the_CommittedEvent_the_given_stream = () => SavedEvent.Name.Should().Be("stream");
		It should_assign_the_CommittedEvent_the_given_event_data = () => SavedEvent.Data.Should().Be("some event data");
		It should_request_to_save_the_given_event = () => The<IEventPersistence>().WasToldTo(er => er.Save(new CommitInfo(1, "stream"), Arg.Any<byte[]>())).OnlyOnce();
	...
	public class When_saving_multiple_events : WithSubject<EventStore>
	{
		...
		It should_assign_the_CommittedEvents_the_given_stream = () => SavedEvents.Should().Contain(evt => evt.Name == "stream");
		It should_assign_the_CommittedEvents_the_given_event_data = () => SavedEvents.Select(e => e.Data.ToString()).Should().ContainInOrder("event 1", "event 2", "event 3");
		It should_request_to_save_the_given_events = () => The<IEventPersistence>().WasToldTo(er => er.Save(Arg.Any<CommitInfo>(), Arg.Any<byte[]>())).Times(3);
	...
	public class When_asked_to_save_with_invalid_arguments : WithSubject<EventStore>
	{
		...
		It should_throw_an_ArgumentException_if_being_passed_a_stream_name_made_of_only_whitespace = () => Subject.Invoking(s => s.Append("    ", "some event data")).ShouldThrow<ArgumentException>();
	}
	...
	public class When_asked_to_save_a_sequence_of_events_containing_a_null : WithSubject<EventStore>
	{
		...
		It should_not_persist_any_event = () => The<ICommitPersistence>().WasNotToldTo(er => er.Save(Arg.Any<CommitInfo>(), Arg.Any<byte[]>()));
	}

	public class DurableStoreReadSetup : WithSubject<EventStore>
	{
		protected static IList<CommitInfo> ReadEvents;

		Establish context = () =>
		{
			ReadEvents = new List<CommitInfo>();
			The<IEventPersistence>().WhenToldTo(er => er.GetInfos()).Return(new[] { new CommitInfo(1, "stream1"), new CommitInfo(2, "stream2"), new CommitInfo(3, "stream1") }.AsQueryable());
			The<IEventPersistence>().When(er => er.Read(Arg.Any<CommitInfo>())).Do(ci => ReadEvents.Add(ci.Arg<CommitInfo>()));
		};
	...
	public class When_asked_to_read_all_the_events_from_all_the_streams : DurableStoreReadSetup
	{
		...
		It should_return_all_the_events = () => ReadEvents.Should().ContainInOrder(new CommitInfo(1, "stream1"), new CommitInfo(2, "stream2"), new CommitInfo(3, "stream1"));
	}
	...
	public class When_asked_to_read_all_the_events_starting_from_a_serial : DurableStoreReadSetup
	{
		...
		It should_return_only_the_events_from_that_serial_onwards = () => ReadEvents.Should().ContainInOrder(new CommitInfo(2, "stream2"), new CommitInfo(3, "stream1"));
	}
	...
	public class When_asked_to_read_the_events_from_a_stream : DurableStoreReadSetup
	{
		...
		It should_return_only_the_events_from_that_stream = () => ReadEvents.Should().ContainInOrder(new CommitInfo(2, "stream2"));
	}
	...
	public class When_asked_to_read_the_events_from_a_stream_and_from_a_serial : DurableStoreReadSetup
	{
		...
		It should_return_only_the_events_from_that_stream_starting_from_that_serial_onwards = () => ReadEvents.Should().ContainInOrder(new CommitInfo(3, "stream1"));
	}
}
